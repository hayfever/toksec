// toksec — live tokens/sec meter for pi's footer status bar.
//
// Install:   cp toksec.ts ~/.pi/agent/extensions/toksec.ts   # then /reload or restart pi
// Test:      pi -e /path/to/toksec.ts
//
...
// --- event shape readers (payload shape varies by provider/version) ---------
  ...
  const noteText = (delta: string, now: number): void => {
    ...
    if (lastTextTs !== null) {
      ...
      if (dt >= MIN_DT && dt <= MAX_DT) {
        ...
        if (!usageSeen) blend(inst); // char estimate only while usage is silent
      }
    }
    lastTextTs = now;
